...
# for a "fairer" comparison we recreate the core of cint(dset()) here
...
# when I used more than around 5e3...
# However, now that I've removed the array version (6/8/22),
# the real CIPerm and the naive.forloop code *can* handle larger ones...
# but it doesn't seem to make much impact on the relative timing diffs.
...
# deltas <- ((-1000*1):(-980*1))/1000 # grid steps of 0.001
...
We also tried using `bench::mark()` to summarize several runtimes together, as well as memory usage. (Not actually run when the vignette knits, in order to avoid needing `bench` as a dependency.)
