...
We also compare against equivalent results in
Ernst, M.D. (2004),
...
Next, let $w_{k,d} = \frac{t_0-t_{k,d}}{k\left(\frac{1}{n}+\frac{1}{m}\right)}$.
...
but records $k$ and $w_{k,d}$ for each permutation.
...
The confidence levels in Ernst (2004) and Nguyen (2009) might differ from ours due to typos or rounding, since it is not possible to achieve exactly 94.90\% or 94.94\% confidence with this method. There are $15 \choose 9$ = 5005 permutations. If we used the 126th, 127th, or 128th lowest and highest $w_{k,d}$ values to get our two-sided CI endpoints, the respective confidence levels would be $1-2\times(126/5005)\approx 0.94965$,
